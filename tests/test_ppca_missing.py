...
def test_ppca_missing() -> bool:
    ...
    exists = xjd.utils.rand.bernoulli(p=0.9, shape=vs.shape)
    missing = 1 + (-1 * exists)
    ...
    model, loc_missing = model.add_node(
        xjd.params.random.Gaussian(shape=vs.shape)
    )
    model, loc_data = model.add_node(
        xjd.transforms.masks.Where(
            loc_exists.result(),
            loc_data_raw.result(),
            loc_missing.param(),
        )
    ...
    model = model.add_node(
        ...
        xjd.constraints.linalg.Orthonormal(loc_weights.param(), T=True),
